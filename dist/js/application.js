...
var Component = (function () {
    ...
    Component.prototype.bind = function ($template, data) {
        ...
        function processEl($el) {
            ...
            try {
                ...
                var cssclass = $this.data("bind-class");
                if (cssclass) {
                    var parts = cssclass.split("?");
                    var classes = parts[1].split(":");
                    var comparison = component.processBindComparison(parts[0], component, data);
                    if (comparison)
                        $this.addClass(classes[0]);
                    else
                        $this.addClass(classes[1]);
                }
                var validate = $this.data("bind-validate");
                if (validate) {
                    var expr_1 = new RegExp(validate);
                    var check_1 = function ($el) {
                        var newVal = $this.val();
                        console.log("Component.bind-validate running: ", { exp: expr_1, val: newVal });
                        if (newVal.match(expr_1))
    ...
    Component.prototype.processBindComparison = function (conf, component, data) {
        if (conf.indexOf("is(") == 0 || conf.indexOf("not(") == 0) {
            ...
            var val2 = variables[1] ? component.textMultiMatch(variables[1], component, data) : "true";
